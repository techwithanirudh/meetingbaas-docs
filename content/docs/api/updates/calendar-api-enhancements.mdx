...
The [`GET /calendar_events`](/docs/api/reference/list_events) endpoint now supports additional filtering parameters:
...
New [`POST /calendars/resync_all`](/docs/api/reference/resync_all_calendars) endpoint to initiate a complete refresh of all connected calendars:
...
### Enhanced Bot Parameter Management

The [`PATCH /calendar_events/{uuid}/bot`](/docs/api/reference/patch_bot_param) endpoint has been renamed from `Patch_bot` to `patch_bot_param` for improved clarity.
