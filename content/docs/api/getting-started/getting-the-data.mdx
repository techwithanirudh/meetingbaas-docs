...
| bot_removed | Bot [removed via API](/docs/api/getting-started/removing-a-bot) or by participant |
| no_attendees | No attendees were present in the meeting |
| no_speaker | No one was speaking for an extended period |
| recording_timeout | The maximum recording duration was reached |
| api_request | The recording was stopped via an API request |
